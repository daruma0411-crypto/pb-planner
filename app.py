...
- **ポジショニング分析のフロー**: まずaxis_x/axis_y未指定で呼び出す → 軸候補リストが返る → ユーザーに軸を選んでもらう → 選択された軸で再度analyze_framework(positioning, axis_x=..., axis_y=...)を呼ぶ
- フレームワーク分析（3C/SWOT/5Forces/価格帯マップ）は競合DBの実データに基づく。推測・ハルシネーションではなくファクトベースで分析する
"""
    ...
    if framework == 'positioning' and (not axis_x or not axis_y):
        ...
        return {
            ...
            "suggested_pairs": [
                ...
                {"x": "有効容量", "y": "重量 (約)", "description": "容量 × 重量（設置性視点）"},
            ],
